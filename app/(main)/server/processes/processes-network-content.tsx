            ...
            return (
              <div key={`network-${connection.protocol}-${connection.localAddress}-${connection.port}-${index}`} className={cn('p-4 hover:bg-muted/50', index < filteredConnections.length - 1 && 'border-b')}>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-muted-foreground">
                  ...
                  {pidUnavailable && <Button htmlType="button" variant="text" onClick={() => handleFindPid(connection)} disabled={findingPid === key} className="flex items-center gap-1.5 text-blue-500 disabled:opacity-50"><Cpu className="h-3.5 w-3.5" />{findingPid === key ? 'Finding PID...' : 'Find PID'}</Button>}
                </div>
              </div>
            );
          })}
          {filteredProcesses.map((process, index) => (
            <div key={`process-${process.pid}`} className={cn('p-4 hover:bg-muted/50', index === 0 && filteredConnections.length > 0 && 'border-t', index < filteredProcesses.length - 1 && 'border-b')}>
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-muted-foreground">
                ...
                <Button variant="text" onClick={() => handleKillProcess(process.pid)} disabled={killingPid === process.pid} className="flex items-center gap-1.5 text-red-500 disabled:opacity-50"><XCircle className="h-3.5 w-3.5" />Kill</Button>
              </div>
            </div>
          ))}
