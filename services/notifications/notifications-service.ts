...
import { notification, type NotificationRecord } from '@neup/logica/notification';
...
export async function getAccountNotifications(accountId: string, offset = 0, limit = NOTIFICATIONS_PAGE_SIZE): Promise<NotificationRecord[]> {
  const response = await notification({ application: process.env.NEUP_APP_ID, appsecret: process.env.NEUP_APP_SECRET })
